...
import { getRitualParticipantById } from 'apiSdk/ritual-participants';
...
function RitualParticipantViewPage() {
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Ritual Participant Detail View
          </Text>
          {hasAccess('ritual_participant', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/ritual-participants/edit/${data?.id}`} passHref legacyBehavior>
              <Button
            ...
            <Box>
              {hasAccess('ritual', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/rituals/view/${data?.ritual?.id}`}>
                      {data?.ritual?.name}
                    </Link>
                  </Text>
                </Flex>
